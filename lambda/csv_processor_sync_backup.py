...
def analyze_item_with_ai(item):
    ...
    try:
        ...
        # Try to call AI API - no fallback to estimates
        try:
            ...
            logger.error(f"AI API failed for item {item['item_number']}: {str(ai_error)}")
            # Return error instead of mock data
            return {
                'estimatedSalePrice': 0,
                'demand': 'Error',
                'salesTime': 'N/A',
                'reasoning': f'AI analysis failed: {str(ai_error)[:100]}',
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Add remaining items - show error for items not analyzed by AI
        remaining_items = items[2:]
        logger.info(f"Skipping {len(remaining_items)} items - AI analysis limited to first 2 items to avoid timeout")
        
        for item in remaining_items:
            # Skip marketplace lookups for non-analyzed items to save time
            # Return error for items not analyzed by AI
            analysis = {
                'estimatedSalePrice': 0,
                'demand': 'Not Analyzed',
                'salesTime': 'N/A',
                'reasoning': 'Item not analyzed - only first 2 items get AI analysis to stay within API Gateway timeout',
                'marketplace': {
                    'amazon': {'available': False, 'price': None, 'url': None},
                    'ebay': {'available': False, 'price': None, 'url': None}
                },
                'image': None,
                'error': True
            }
